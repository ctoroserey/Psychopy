...
expName = 'physTask'  # from the Builder filename that created this script
...
trial_length = 10 # trial length in secs
...
for k in iti_order:
    ...
    itiClock = core.Clock()
    counter = 0 # to help distribute rewardOrder more evenly
    # Begin trials
    while itiClock.getTime() < blockLength:
        ...
        phys_response = phys_cond(win,trial_length)
        ...
        elif phys_response == 3: # if failed
            needs_reward = False
            # update logs
            iti_log.append(k)
            rwd_log.append(j)
