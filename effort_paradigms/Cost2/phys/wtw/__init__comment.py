...
def createStimulusObjects(win,frameDur):
    ...
    # time left text: TextBox version
    # TextBox should be faster to update than TextStim, but seems buggy.
    # e.g. only displaying the first few letters.
    # if using this, add .draw() command (AutoDraw not supported)
    # timeLeftText = visual.TextBox(window=win,
    #     text='Time left: 00:00', units='pix', font_name='Courier New',
    #     pos=[0, 200], font_size=28, font_color=(1, 1, 1), bold=True,
    #     align_horz='center', size=(800,100), grid_color=(.5,.5,.5),
    #     background_color=(-0.5,-0.5,-0.5), interpolate=True, grid_horz_justification='center')
    ...
    # total earned text: TextBox version (see above)
    # totalEarnedText = visual.TextBox(window=win,
    #     text='Total earned: $00.00',  units='norm', font_name='Courier New',
    #     pos=[0, -0.7], font_size=28, font_color=(1, 1, 1), bold=True,
    #     align_horz='center', size=(1.8,0.1))
    ...
    meterMarker.size *= 1
    meterNeedle.size *= 1
    #meterDial.pos += (-0.2, -0.325) # reposition all meter components
    #meterMarker.pos += (-0.2, -0.325)
    #meterNeedle.pos += (-0.2, -0.325)
...
def showTrials(blockParams, stimObjects, thisExp, expObjects):
    ...
    # Claudio: this is the main thing that I should modify. Everything else can be
    # commented out at presentation time.
    #blockIdx = blockParams['blockIdx']
    #drawSample = blockParams['drawSample']
    #checkSellResponse = blockParams['checkSellResponse']
    ...
    #tokenColor = blockParams['tokenColor']
    #totalEarned = blockParams['totalEarned']
    #fbkDuration = blockParams['fbkDuration'] # try to remove this
    #itiDuration = blockParams['itiDuration']

    # unpack inputs from stimObjects
    frameDur = stimObjects['frameDur']
    #message = stimObjects['message']
    #token = stimObjects['token']
    #tokenText = stimObjects['tokenText']
    readyText = stimObjects['readyText'] # Claudio: could change this to display the prospective reward.
    #soldText = stimObjects['soldText']
    #progressBar = stimObjects['progressBar']
    #progressBarBase = stimObjects['progressBarBase']
    #timeLeftText = stimObjects['timeLeftText']
    #totalEarnedText = stimObjects['totalEarnedText']
    #progBarMaxTime = stimObjects['progBarMaxTime']
    #progBarMaxLength = stimObjects['progBarMaxLength']
    #tokenRewardColor = stimObjects['tokenRewardColor']
    #tokenHoleColor = stimObjects['tokenHoleColor']
    ...
    win = expObjects['win']
    #blockClock = expObjects['blockClock']
    ...
    # flipTimeIsReadyOnsetTime = False
    # flipTimeIsTokenOnsetTime = False
    # flipTimeIsRewardOnsetTime = False
    # flipTimeIsFeedbackOnsetTime = False
    # feedbackOnsetTime = None
    # frameTimes = [] # frame time list (for testing)

    # initial state of the display
    #progressBarBase.setAutoDraw(True)
    #timeLeftText.setAutoDraw(True)
    #totalEarnedText.setAutoDraw(True)
    #token.fillColor = tokenHoleColor
    #token.lineColor=[0,0,0]
    #token.setAutoDraw(True)
    #totalEarnedText.setText('Total earned: ${:.2f}'.format(totalEarned/100))
    ...
    # set and store the state of the random number generator for this block
    #np.random.seed(None) # by default uses /dev/urandom or clock
    #blockParams['rngState'] = np.random.get_state() # for logging Claudio: unneeded
    ...
    # while loop for the duration of the block
    ...
    #while (blockClock.getTime() < blockDuration) & (trialIdx <= trialLimit): # Claudio: remove this to generate one trial only?
    while (trialClock.getTime() < trialLimit): #(responseOutput['responded'] == False): #
        ...
        if (stateNow == 'iti') and (timeNow >= (nextEventTime - frameDur)) and responseOutput['responded']: #and (trialIdx < trialLimit):
        ...
        if (stateNow == 'iti') and (timeNow >= (nextEventTime - frameDur)):
            stateNow = 'waiting'
            #drawSampleOutput = wtw.drawSample.discreteUnif16(seq) #drawSample(seq) # wtw.drawSample.discreteUnif16(seq)
            #seq = drawSampleOutput['seq'] # unpack
            #scheduledDelay = drawSampleOutput['nextDelay'] # unpack
            #nextEventTime = 15 # clock time when the token will mature Claudio: probably unneeded, but it could be the handling time
            #token.fillColor = tokenColor
            #token.lineWidth = 2
            #token.lineColor = [1,1,1]
            #token.setAutoDraw(True)
            #progressBar.setAutoDraw(True)
            #tokenValue = rwdLo
            #tokenText.setText('{}'.format(tokenValue) + rwdUnit)
            #tokenText.setAutoDraw(True)
            #trialIdx += 1 # increment trial number
            #if trialIdx > trialLimit: continue # prevent final screen update if exiting
            event.clearEvents() # ITI key presses, if any, should not carry over
            #flipTimeIsTokenOnsetTime = True # will store next flip as token onset time
            # initialize timing variables (which should not carry over from trial to trial)
            #   (note: feedbackOnsetTime is not included here, it *should* carry over)
            #readyOnsetTime = None
            #tokenOnsetTime = None
            #rewardOnsetTime = None
            #keyTime = None
        ...
        # state change from waiting to matured
        # Claudio: probably can ditch
        #elif (stateNow == 'waiting') and (tokenValue == rwdLo) and (timeNow >= (nextEventTime - frameDur)):
        #    stateNow = 'matured' # no update to nextEventTime, since the next event is response-triggered
        #    token.fillColor = tokenRewardColor
        #    tokenValue = rwdHi
        #    tokenText.setText('{}'.format(tokenValue) + rwdUnit)
        #    #tokenText.setAutoDraw(True)
        #    flipTimeIsRewardOnsetTime = True # will store next flip as reward onset time

        # state change from feedback to iti
        #elif (stateNow == 'feedback') and (timeNow >= (nextEventTime - frameDur)):
        #    stateNow = 'iti'
            #nextEventTime = nextEventTime + itiDuration
            #soldText.setAutoDraw(False)
            #tokenText.setAutoDraw(False)
            #token.fillColor = tokenHoleColor
            #token.lineColor=[0,0,0]

            # end of if-else statement for time-dependent state changes

        # response-dependent state change (from waiting or matured to feedback)
        if stateNow in ['waiting', 'matured']:

            # if a sell response was recorded
            # Claudio: if quit
            if (responseOutput['responded'] == True):
                #stateNow = 'feedback'
                #soldText.setAutoDraw(True) # Claudio: just as a check, remove afterwards
                #progressBar.setAutoDraw(False)
                #flipTimeIsFeedbackOnsetTime = True # will store next flip as feedback onset time
                #responseClockTime = responseOutput['responseClockTime']
                #nextEventTime = responseClockTime + fbkDuration
                #trialEarnings = 5
                #totalEarned += trialEarnings
                #totalEarnedText.setText('Total earned: ${:.2f}'.format(totalEarned/100))
                #progressBar.width = 0
        ...
        #if stateNow in ['waiting', 'matured']:
        #    continue
            # if tokenOnsetTime == None: # first flip of each trial
            #     progLength = 0
            # else:
            #     progLength = progBarMaxLength*(timeNow - tokenOnsetTime)/progBarMaxTime
            #progressBar.width = progLength
            # comment the next 2 lines to have the bar grow from center; uncomment to have it grow from the left.
            #progPos = (progLength - progBarMaxLength)/2
            #oldPos = progressBar.pos
            #progressBar.pos = (progPos,oldPos[1])

        # update display of time remaining in block
        # timeLeft = np.ceil(trialLimit - timeNow) # in seconds, rounded up
        # timeLeftMin = int(np.floor(timeLeft/60)) # whole minutes part
        # timeLeftSec = int(np.mod(timeLeft,60)) # whole seconds part
        # timeLeftText.setText('Time left: {:d}:{:02d}'.format(timeLeftMin, timeLeftSec))
        ...
        # we always flip.
        win.flip()
        # log the flip time if necessary
        # flipTime = trialClock.getTime()
        # if flipTimeIsReadyOnsetTime:
        #     readyOnsetTime = flipTime
        #     flipTimeIsReadyOnsetTime = False
        # if flipTimeIsTokenOnsetTime:
        #     tokenOnsetTime = flipTime
        #     flipTimeIsTokenOnsetTime = False
        # if flipTimeIsRewardOnsetTime:
        #     rewardOnsetTime = flipTime
        #     flipTimeIsRewardOnsetTime = False
        # if flipTimeIsFeedbackOnsetTime:
        #     feedbackOnsetTime = flipTime
        #     flipTimeIsFeedbackOnsetTime = False

        # store frame times for testing
        # frameTimes.append(flipTime)
    ...
    # for testing, save out frame times
    # csvfname = expObjects['filename'] + '_frameTimes-bk' + str(blockIdx) + '.txt'
    # with open(csvfname, 'wb') as csvfile:
    #     cw = csv.writer(csvfile)
    #     cw.writerow(frameTimes)
    ...
    #soldText.setAutoDraw(False)
    #progressBar.setAutoDraw(False)
    #progressBarBase.setAutoDraw(False)
    #timeLeftText.setAutoDraw(False)
    #totalEarnedText.setAutoDraw(False)
    ...
    if LabJack != 0:
        wtw.handgrip.trialEndSignal(LabJack)
    # package outputs
    #blockParams['totalEarned'] = totalEarned
    #trialOutput = {'thisExp':thisExp, 'blockParams':blockParams}
